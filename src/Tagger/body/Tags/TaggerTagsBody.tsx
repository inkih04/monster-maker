		...
		<div className="tagger-body--scroll">
			...
			<ul className="tagger-kv--list">
				{entries.map((entry, index) => {
					const isLocalFile = entry.path.toLocaleLowerCase().endsWith('.local');

					return (
						<React.Fragment key={entry.id}>
							{index > 0 && <li className="tagger-kv--row-separator" />}
							<li className="tagger-kv--row">
								<input
								...
								{!isLocalFile ? (
									...
									<span className="tagger-kv--col-spacer" style={{ width: '24px' }} />
								)}
							</li>
						</React.Fragment>
					);
